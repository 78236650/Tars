...
### 工作流与问数
- ✅ **双层状态** — 数据源 `schema_snapshot.insight.workflow` + 会话 `metadata.insight`
- ✅ **WorkflowStrip** — Chat 内鉴数进度、失败重试、`pending_question` 接续（H5）
- ✅ **MetricQaEngine** — `POST /ask`、`/ask/stream`、few-shot 问数日志（H4）
- ✅ **MetricAnswerCard** — 三档口径、👍👎、采用官方口径

### 治理
- ✅ **Adoption** — `POST /metrics/adopt`、版本 bump、H3 自动降级
- ✅ **insight_analyst** — 8 个 `insight_*` 工具，禁止 chart/python

### 运维与 GA
- ✅ **W1 运维台** — Schema 速览抽屉；LLM 迁至 **`/admin/insight/llm`**
- ✅ **`feature_flags.chat_first_enabled: true`**（默认 GA）
- ✅ **评测集** — `pytest -m insight_eval`（10 题，门槛 ≥80%）
- 📄 部署约束见 [insightforge-deploy.md](../04-运维文档/insightforge-deploy.md)（SSE / workers=1 或 sticky）
