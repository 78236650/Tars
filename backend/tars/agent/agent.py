...
from . import prompt_builder as _pb
...
class AgentV2:
    ...
    def _build_system_prompt(self, user_content: str, session_id: str,
        ...
        return _pb.build_system_prompt(self, user_content, session_id, tenant_id,
                                        memory_context, scoped_memory_manager, cmd_result,
                                        knowledge_context)

    def _build_static_prompt(self, user_content: str, session_id: str,
                              cmd_result, tenant_id: str = "default") -> str:
        return _pb.build_static_prompt(self, user_content, session_id, cmd_result, tenant_id)

    def _route_skills_for_message(self, user_content: str, tenant_id: str = "default",
                                   scene_intent=None, scene_keywords=None):
        return _pb.route_skills_for_message(self, user_content, tenant_id,
                                             scene_intent, scene_keywords)

    def _build_skill_injection(self, matched) -> str:
        return _pb.build_skill_injection(self, matched)

    def _record_skill_activations(self, matched, session_id: str = "default") -> None:
        return _pb.record_skill_activations(self, matched, session_id)

    def _record_routing_candidates(self, session_id: str, candidates) -> None:
        return _pb.record_routing_candidates(self, session_id, candidates)

    def _build_memory_prompt(self, memory_context: str, knowledge_context: str = "") -> str:
        return _pb.build_memory_prompt(self, memory_context, knowledge_context)

    async def _build_knowledge_context(self, user_content: str, tenant_id: str,
                                        channel, session_id: str) -> str:
        return await _pb.build_knowledge_context(self, user_content, tenant_id, channel, session_id)
