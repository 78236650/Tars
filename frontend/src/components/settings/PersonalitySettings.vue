...
const presetPersonalities = [
 { key: 'professional', params: { honesty: 0.9, humor: 0.3, initiative: 0.8, empathy: 0.6, formality: 0.8, creativity: 0.5, conciseness: 0.8, technical_depth: 0.9, curiosity: 0.7, skepticism: 0.6 } },
 { key: 'friendly', params: { honesty: 0.8, humor: 0.7, initiative: 0.6, empathy: 0.9, formality: 0.3, creativity: 0.7, conciseness: 0.5, technical_depth: 0.4, curiosity: 0.8, skepticism: 0.2 } },
 { key: 'creative', params: { honesty: 0.7, humor: 0.6, initiative: 0.9, empathy: 0.7, formality: 0.4, creativity: 0.9, conciseness: 0.4, technical_depth: 0.5, curiosity: 0.9, skepticism: 0.3 } },
 { key: 'scholar', params: { honesty: 0.9, humor: 0.2, initiative: 0.7, empathy: 0.5, formality: 0.9, creativity: 0.6, conciseness: 0.9, technical_depth: 0.8, curiosity: 0.8, skepticism: 0.7 } }
];
...
const paramLabel = (key: string) => t(`personalitySettings.params.${key}`);
...
<template>
  <div class="max-w-4xl mx-auto">
    <div class="bg-slate-800 rounded-xl p-6">
      ...
      <div class="mb-6">
        <label class="block text-sm font-medium text-slate-300 mb-3">{{ t('personalitySettings.preset') }}</label>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          >
            {{ t(`personalitySettings.presets.${preset.key}`) }}
          </button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        <div v-for="(value, key) in params" :key="key" class="bg-slate-700 rounded-lg p-4">
          <label class="block text-sm text-slate-400 mb-2">{{ paramLabel(String(key)) }}</label>
          <input
            type="range"
      ...
      <div class="mb-6">
        <label class="block text-sm font-medium text-slate-300 mb-3">{{ t('personalitySettings.communicationStyle') }}</label>
        <textarea
      ...
      <div class="mb-6">
        <label class="block text-sm font-medium text-slate-300 mb-3">{{ t('personalitySettings.behaviorRules') }}</label>
        <div class="space-y-2">
          <div
